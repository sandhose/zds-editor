...
test("new TextareaAdapter()", assert => {
  ...
  assert.throws(
    () => new TextareaAdapter("beep"),
    "throws when argument is not a DOMNode"
  );
  assert.throws(
    () => new TextareaAdapter(document.createElement("div")),
...
test("TextareaAdapter#destroy", assert => {
  ...
  assert.equal(
    textarea.parentNode,
    container,
    "textarea is reattached to its original parentNode"
  );
  assert.notOk(
    container.contains(wrapper),
    "wrapperNode is dettached from DOM"
  );
  assert.equal(
    container.children.length,
    1,
    "other nodes are dettached from the container"
  );
...
test("TextareaAdapter#getLines", assert => {
  ...
  assert.deepEqual(
    adapter.getLines(),
    ["beep"],
    "returns an array with one line"
  );
  adapter.setText("beep\nboop");
  assert.deepEqual(
    adapter.getLines(),
    ["beep", "boop"],
    "returns an array with the lines of text"
  );
...
test("TextareaAdapter#getIndexFromPos", assert => {
  ...
  adapter.setText("beep\nboop");
  assert.equal(
    adapter.getIndexFromPos(new Pos({ line: 0, ch: 0 })),
    0,
    "returns the correct index on the first line, first char"
  );
  assert.equal(
    adapter.getIndexFromPos(new Pos({ line: 0, ch: 4 })),
    4,
    "returns the correct index on the first line, last char"
  );
  assert.equal(
    adapter.getIndexFromPos(new Pos({ line: 1, ch: 0 })),
    5,
    "returns the correct index on another line, first char"
  );
  assert.equal(
    adapter.getIndexFromPos(new Pos({ line: 1, ch: 4 })),
    9,
...
test("TextareaAdapter#focus", assert => {
  ...
  assert.notEqual(
    ...
    textarea,
    "textarea should not have the focus"
  );
  ...
  assert.equal(
    document.activeElement,
    textarea,
    "textarea should have the focus"
  );
...
test("TextareaAdapter#handleKeydown", assert => {
  ...
  assert.equal(
    handler.called,
    false,
    "should not emit action when keymap not registered"
  );
  handler.reset();
  ...
  assert.ok(
    ...
    "should emit action with complex keymap"
  );
  handler.reset();
