...
test("new Pos()", assert => {
  ...
  assert.throws(
    () => new Pos({ line: 0 }),
    "should throw if ch is not defined"
  );
  assert.throws(() => new Pos({ ch: 0 }), "should throw if ch is not defined");
  assert.throws(
    () => new Pos({ line: "beep" }),
    "should throw if line is not a number"
  );
  assert.throws(
    () => new Pos({ ch: "boop" }),
    "should throw if ch is not a number"
  );

  assert.doesNotThrow(
    () => new Pos({ line: 0, ch: 42 }),
...
test("Pos.compare", assert => {
  ...
  assert.plan(4);
  assert.true(
    Pos.compare(pos1, pos2) < 0,
    "is negative if a is before b (on the same line)"
  );
  assert.true(
    Pos.compare(pos2, pos3) < 0,
    "is negative if a is before b (on different lines)"
  );
...
test("new Range()", assert => {
  ...
  assert.throws(
    () => new Range(),
    "should throw if no start & end are defined"
  );
  assert.throws(
    () => new Range("beep", "boop"),
    "should throw if start or end are not Pos"
  );
  assert.doesNotThrow(
...
test("Range.fromCmRange", assert => {
  ...
  assert.deepEqual(
    Range.fromCmRange({ anchor: pos1, head: pos2 }),
    range,
    "transforms from codemirror's range"
  );
  assert.deepEqual(
    Range.fromCmRange({ anchor: pos2, head: pos1 }),
    range,
